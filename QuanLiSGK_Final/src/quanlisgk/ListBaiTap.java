...
public class ListBaiTap {
    ...
    int d=-1;
    for(int i = 0; i < list2.size(); i++){
        if(maSach.equals(list2.get(i).getMaSach())){
        position = true;
        d=i;
        do{
            ...
            System.out.println("Ma sach : "+ maSach +", Ngay nhap : "+ngayNhapBT+", Xuat ngay :"+nXuat+", So luong xuat kho : "+xKho);
            list2.get(i).setSoLuong(list2.get(i).getSoLuong());
                        try {
        FileWriter fw = new FileWriter("PhieuxuatkhoBT.txt",true);
        BufferedWriter bw = new BufferedWriter(fw);
        bw.write("Ma sach : "+ maSach +", Ngay nhap: "+ngayNhapBT+", Ngay xuat : "+nXuat+", So luong xuat : "+xKho);
        bw.newLine();
        bw.close();
       } catch (Exception e) {
       }
